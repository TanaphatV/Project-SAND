...
void drawStar(glm::mat4 transform);
void drawMoon(glm::mat4 transform);
void render();
...
int main(int argc, char* argv[])
{
    ...
    while (!quit)
    {
        ...
        glClearColor(0.0f, 76.0f/255.0f, 153.0f/255.0f, 1.f);
        // Clear color buffer
        ...
        float zoomRatio = 1 / (float(zoomLevel) / 5.0f);

        glm::mat4 projectionMatrix = glm::ortho(left * zoomRatio, right * zoomRatio, down * zoomRatio, up * zoomRatio);
...
void render() {
    ...
    drawSquare(glm::vec3(0.0f, 102.0f/255.0f, 51.0f/255.0f), matrix);

    //top moon
    matrix = glm::translate(glm::mat4(1.0f), glm::vec3(0.2f, 2.2f, 0.0f));
    drawMoon(matrix);
    ...
    {
        //top star
        matrix = glm::translate(glm::mat4(1.0f), glm::vec3(0.8f + i * posOffset, 2.5f, 0.0f));
        matrix = glm::scale(matrix, glm::vec3(0.3f, 0.3f, 1.0f));
        drawStar(matrix);

        //left mountain
        matrix = glm::translate(glm::mat4(1.0f), glm::vec3(-1.0f + i * posOffset, 0.0f, 0.0f));
        matrix = glm::scale(matrix, glm::vec3(1.5f, 1.5f, 0.0f));
        drawTriangle(glm::vec3(0.0f, 153.0f / 255.0f, 153.0f / 255.0f), matrix);

        //right mountain
        matrix = glm::translate(glm::mat4(1.0f), glm::vec3(0.0f + i * posOffset, 0.0f, 0.0f));
        drawTriangle(glm::vec3(0.0f, 153.0f / 255.0f, 153.0f / 255.0f), matrix);
    }
...
void drawStar(glm::mat4 transform) 
{
    glm::mat4 pos = glm::translate(transform, glm::vec3(-1.0f, -0.6f, 0.0));
    drawTriangle(glm::vec3(1.0f, 1.0f, 0.0f), pos);

    pos = glm::translate(transform, glm::vec3(1.0f, 0.0f, 0.0));
    pos = glm::rotate(transform, (float)M_PI, glm::vec3(0.0f, 0.0f, 1.0f));
    drawTriangle(glm::vec3(1.0f, 1.0f, 0.0f), pos);
}

void drawMoon(glm::mat4 transform) 
{
    drawCircle(glm::vec3(1.0f, 1.0f, 0.5f), transform);
    glm::mat4 pos = glm::translate(transform, glm::vec3(0.3f, 0.15f, 0.0));
    drawCircle(glm::vec3(0.0f, 76.0f / 255.0f, 153.0f / 255.0f), pos);
}
